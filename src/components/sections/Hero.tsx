...
export function Hero() {
  return (
    <section id="home" className="relative min-h-svh w-full overflow-hidden">
      {/* Brilho sutil + águia desenhada à mão (mesma da embalagem) */}
      <div
        aria-hidden
        className="absolute inset-0 -z-10 [background:radial-gradient(80%_60%_at_70%_15%,rgba(200,184,154,0.08),transparent_60%)]"
      />
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -right-16 top-20 -z-10 w-[60vw] max-w-2xl opacity-[0.06] sm:opacity-[0.08] lg:-right-10"
        initial={{ opacity: 0, scale: 1.05 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.4, ease }}
      >
        <Image
          src="/img/voodesenho-removebg-preview.png"
          alt=""
          width={760}
          height={760}
          priority
